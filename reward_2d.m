...
% function iti_vector = assign_iti_with_only_large_and_mid(reward)
%     % ITI sizes
%     iti_size_v = [0, 45, 120]; % ITI sizes
% 
%     % Indices for larger and smaller rewards
%     largerewardind = find(reward == 3); % Indices for larger reward
%     smallrewardind = find(reward == 2); % Indices for small reward
%     
%     % Initialize iti_vector
%     iti_vector = zeros(length(reward), 1);
% 
%     % Randomly assign 45 and 120 ITIs (consecutive block approach)
%     num_consecutive_blocks = 5; % Number of consecutive 45 and 120 ITIs (5 blocks each)
%     
%     % Assign 5 blocks of consecutive 45 ITIs
%     for i = 1:num_consecutive_blocks
%         available_indices_45 = find(iti_vector == 0); % Find all unassigned positions
%         index_45_start = available_indices_45(randi(length(available_indices_45))); % Random starting index
%         iti_vector(index_45_start:index_45_start+4) = 45; % Assign consecutive 45 (5 blocks)
%     end
%     
%     % Assign 5 blocks of consecutive 120 ITIs
%     for i = 1:num_consecutive_blocks
%         available_indices_120 = find(iti_vector == 0); % Find all unassigned positions
%         index_120_start = available_indices_120(randi(length(available_indices_120))); % Random starting index
%         iti_vector(index_120_start:index_120_start+4) = 120; % Assign consecutive 120 (5 blocks)
%     end
%     
%     % Assign 20 consecutive random ITIs of 45 or 120
%     random_45_120_indices = find(iti_vector == 0); % Get all unassigned positions
%     index_20_start = random_45_120_indices(randi(length(random_45_120_indices))); % Random start position for 20 consecutive
% 
%     % Assign 20 consecutive ITIs, randomly alternating between 45 and 120
%     for i = 0:19
%         if mod(i, 2) == 0
%             iti_vector(index_20_start + i) = 45;
%         else
%             iti_vector(index_20_start + i) = 120;
%         end
%     end
% 
%     % Ensure at least two regular ITIs between small and large rewards
%     % 10% small rewards and 10% large rewards
%     total_rewards = length(reward);
%     num_small_rewards = round(total_rewards * 0.1);
%     num_large_rewards = round(total_rewards * 0.1);
%     
%     % Select random positions for small and large rewards
%     available_indices = find(iti_vector == 0);
%     small_reward_positions = randsample(available_indices, num_small_rewards);
%     large_reward_positions = randsample(setdiff(available_indices, small_reward_positions), num_large_rewards);
% 
%     % Assign small and large rewards
%     iti_vector(small_reward_positions) = 45; % Assign small rewards as 45
%     iti_vector(large_reward_positions) = 120; % Assign large rewards as 120
% 
%     % Ensure that there's at least two regular ITIs (0s) between each small and large reward
%     for i = 1:length(small_reward_positions)
%         % Ensure that each large reward is at least 2 positions away from small rewards
%         large_reward_positions = setdiff(large_reward_positions, ...
%             small_reward_positions(i) - 2 : small_reward_positions(i) + 2);
%     end
%     
%     % If any issues remain, randomly assign additional 0s to make sure the conditions hold
% end


%%
% 
% %% Function to assign ITI based on the block type (rule)
% function iti = assign_iti(rule_type, reward)
%     % Assign ITIs based on the reward types using the selected rule
%     iti = zeros(1, numel(reward));  % Default to 0
% 
%     switch rule_type
%         case 1
%             % Rule 1: Regular = 0s, Large = 45s, Small = 120s
%             iti(reward == 1) = 0;    % Regular rewards get iti 0
%             iti(reward == 2) = 45;   % Large rewards get iti 45
%             iti(reward == 3) = 120;  % Small rewards get iti 120
%             % Ensure ITI rules: 45s and 120s follow at least 2 consecutive 0s
%             iti = enforce_iti_constraint(iti);
% 
%         case 2
%             % Rule 2: Same as Rule 1 but 45s/120s iti can be consecutive
%             iti(reward == 1) = 0;
%             iti(reward == 2) = 45;
%             iti(reward == 3) = 120;
%             % No need to enforce any further constraints
% 
%         case 3
%             % Rule 3: Same as Rule 1 but large rewards come consecutively
%             iti(reward == 1) = 0;
%             iti(reward == 2) = 45;
%             iti(reward == 3) = 120;
%             % Ensure large rewards are consecutive
%             reward = place_large_rewards_consecutively(reward);
%             iti = enforce_iti_constraint(iti);
% 
%         case 4
%             % Rule 4: Same as Rule 1 but small rewards come consecutively
%             iti(reward == 1) = 0;
%             iti(reward == 2) = 45;
%             iti(reward == 3) = 120;
%             % Ensure small rewards are consecutive
%             reward = place_small_rewards_consecutively(reward);
%             iti = enforce_iti_constraint(iti);
% 
%         case 5
%             % Rule 5: Same as Rule 3 but large rewards always have iti of 45s
%             iti = assign_iti(3, reward); % Rule 3 logic
%             iti(reward == 2) = 45;  % Enforce 45s ITI for large rewards
% 
%         case 6
%             % Rule 6: Same as Rule 4 but small rewards always have iti of 45s
%             iti = assign_iti(4, reward); % Rule 4 logic
%             iti(reward == 3) = 45;  % Enforce 45s ITI for small rewards
%     end
% end
% 
% %% Function to enforce ITI constraints (ensure 45s and 120s follow at least two 0s)
% function iti = enforce_iti_constraint(iti)
%     for t = 3:length(iti)
%         if (iti(t) == 45 || iti(t) == 120) && ~(iti(t-1) == 0 && iti(t-2) == 0)
%             iti(t) = 0;  % Reset ITI to 0 if the constraint is violated
%         end
%     end
% end
% 
% %% Function to place large rewards consecutively (for Rule 3)
% function reward = place_large_rewards_consecutively(reward)
%     % Ensure that large rewards (2) come consecutively
%     idx_large = find(reward == 2);
%     if numel(idx_large) == 3
%         reward(idx_large(2)) = 2;  % Ensure 3 large rewards stay together
%     end
% end
% 
% %% Function to place small rewards consecutively (for Rule 4)
% function reward = place_small_rewards_consecutively(reward)
%     % Ensure that small rewards (3) come consecutively
%     idx_small = find(reward == 3);
%     if numel(idx_small) == 3
%         reward(idx_small(2)) = 3;  % Ensure 3 small rewards stay together
%     end
% end
% 
% 
